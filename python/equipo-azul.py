...
class moverPorteroAzul(object):
    def __init__(self):
        rospy.Subscriber("PAzul/camera1", Image, self.image_callback)
        client.subscribeTopic("/PAzul/pose", self.onPoseMessage)
        ...
        try:
            ...
            if x_pelota is not None:
                if x_pelota > 340 and self.y_robot > -0.7:
                    ...
                    sendRobotTwistSetpoint(client, "PAzul", 0.3, 0, 0)
                elif x_pelota <300 and self.y_robot < 0.7:
                    #print("Mover a la derecha")
                    sendRobotTwistSetpoint(client, "PAzul", -0.3, 0, 0)
                else:
                    #print("Parar robot")
                    sendRobotTwistSetpoint(client, "PAzul", 0, 0, 0)
            else:
                sendRobotTwistSetpoint(client, "PAzul", 0, 0, 0)
                #print("Pelota no visible, Parar robot")
            cv2.imshow('PAzul-Deteccion de colores', cv2_img)
            cv2.imwrite('/PAzul/camera1.jpeg', cv2_img)
            cv2.waitKey(50)  
